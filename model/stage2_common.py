...
def pixel_to_series(pixel, zero_mv, length):
	...
	for j in [0, 1, 2, 3]:
		...
		# find max (upper envolope)
		amax = p.argmax(0)
		amin = H - 1 - p[::-1].argmax(0)  # crop H=1696
		mask = amax >= zero_mv[j]

		s1 = mask * amax + (1 - mask) * amin
		s  = amax

		# find missing
		miss = ((p > 0.1).sum(0) == 0)  # * 100 + zero_mv[j]
		s[miss] = zero_mv[j]
		series.append(s)
...
def filter_series_by_limits(
        ...
        """
        Clip amplitudes per quarter of the trace for channels 0–2,
        and globally for channel 3 (II-rhythm).
        series: (C, L)
        """
        C, L = series.shape

        # Clip II-rhythm a bit tighter
        series[3] = np.clip(series[3], -1, 1)

        # Clip short-lead stacks per time-quarter
        for j in [0, 1, 2]:
                for i in range(4):
                        i0 = i * (L // 4)
